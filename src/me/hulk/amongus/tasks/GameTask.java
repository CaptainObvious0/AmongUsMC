    ...
    public void run() {
        for (GamePlayer player : game.getPlayersInGame().values()) {
            for (GamePlayer player1 : game.getPlayersInGame().values()) {
                if (player.getPlayer().getLocation().distance(player1.getPlayer().getLocation()) > player.getPlayerVision()) player.getPlayer().hidePlayer(AmongUs.getInstance(), player1.getPlayer());
            }
        }
    }
